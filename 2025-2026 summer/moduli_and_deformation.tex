...
\begin{itemize}
    ...
    \item morphisms $(u,v) \to (u',v')$ to be pairs of arrows $f:u\to u'$, $g:v\to v'$ such that $F(f)=G(g)$.
\end{itemize}
...
\begin{itemize}
    ...
    \item morphisms $(u,v,\alpha)\to (u',v',\alpha')$ to be pairs $(f,g)$ where $f: F(u)\to F(u'),g: G(v)\to G(v')$ such that the following diagram commutes.
    \[\begin{tikzcd}
        F(u) \rar["F(f)"] \dar["\alpha"] & F(u') \dar["\alpha'"]\\
        G(v) \rar["G(g)"] & G(v')
    \end{tikzcd}\] 
\end{itemize}
