...
{\color{red} Now let's apply our knowledge.} Repeating the definition in our special case, the first order deformations are the groupoid, which is the preimage $(i^*)^{-1}(x)$. Denoting the unique object $\bullet\in \mathbbm{1}$, our definition of the fiber product $F(\Spec D)\times_{F(\Spec k)} \mathbbm{1}$ specializes to the category with
\begin{itemize}
...
In this setup, the groupoid of first order deformations of $\cF \in \Coh_X(\Spec k)$ will be a pair $(\cF',\alpha)$, where $\cF'\in \Coh_X(\Spec D)$ is a coherent sheaf on $X\times_k \Spec D$ flat over $\Spec D$, and for $i:\Spec k\to \Spec D$ the inclusion oft the closed point, $\alpha$ is an isomorphism
