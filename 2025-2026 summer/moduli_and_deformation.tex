...
\begin{rmk}
    We can contrast the example of $\Coh_X$ to the moduli space of closed subschemes, i.e., the Hilbert scheme. In that case, we technically are ``modding out'' by an equivalence relation in order for $\Hilb^P_X$ to be a functor to $\Set$ rather than $\Groupoid$.
    
    The stacks/higher category perspective is that one should remember automorphisms, but in this special case it turns out to be harmless, since all automorphisms of a closed subscheme $Z\hookrightarrow X$ (i.e., a surjection $\cO_X\to \cO_Z$) are trivial when the automorphism has to commute with inclusion. This is not the case for an abstract coherent sheaf.
\end{rmk}
